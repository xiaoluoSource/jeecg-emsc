...
public class SysPermissionServiceImpl extends ServiceImpl<SysPermissionMapper, SysPermission> implements ISysPermissionService {
	...
	//加个判断，如果登录人为admin也只能看到添加后的菜单，和后台管理区分开来
	@Override
	public List<SysPermission> queryByUser(String userId,String param) {
		...
		//用户端判断,如果为admin也不让看开发菜单
		List<SysPermission> permissionList1 = new ArrayList<>();
		if (!StringUtils.isBlank(param) && "view".equals(param) && "e9ca23d68d884d4ebb19d07889727dae".equals(userId)) {
			for (SysPermission sysPermission : permissionList) {
				if (!(
						(sysPermission.getId().equals("1455100420297859074") || sysPermission.getId().equals("d7d6e2e4e2934f2c9385a623fd98c6f3") ||
								sysPermission.getId().equals("1661572802889007106") || sysPermission.getId().equals("1674708136602542082") ||
								sysPermission.getId().equals("1439398677984878593") || sysPermission.getId().equals("1443390062919208961") ||
								sysPermission.getId().equals("1447763172274495489") || sysPermission.getId().equals("1438108196993622018") ||
								sysPermission.getId().equals("1438108187455774722") || sysPermission.getId().equals("1438108225451974658"))|| sysPermission.getId().equals("1438108176273760258")) ||
								(sysPermission.getParentId().equals("1455100420297859074") || sysPermission.getParentId().equals("d7d6e2e4e2934f2c9385a623fd98c6f3") ||
										sysPermission.getParentId().equals("1661572802889007106") || sysPermission.getParentId().equals("1674708136602542082") ||
										sysPermission.getParentId().equals("1439398677984878593") || sysPermission.getParentId().equals("1443390062919208961") ||
										sysPermission.getParentId().equals("1447763172274495489") || sysPermission.getParentId().equals("1438108196993622018") ||
										sysPermission.getParentId().equals("1438108187455774722") || sysPermission.getParentId().equals("1438108225451974658")|| sysPermission.getParentId().equals("1438108176273760258")
				)) {
					permissionList1.add(sysPermission);
				}
			}
		}else if ("dev".equals(param)){
			for (SysPermission sysPermission : permissionList) {
				if ((
						(sysPermission.getId().equals("1455100420297859074") || sysPermission.getId().equals("d7d6e2e4e2934f2c9385a623fd98c6f3") ||
								sysPermission.getId().equals("1661572802889007106") || sysPermission.getId().equals("1674708136602542082") ||
